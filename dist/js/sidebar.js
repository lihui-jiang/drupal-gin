  ...
  "./js/sidebar.js": function () {
    ((Drupal, drupalSettings, once) => {
      const toolbarVariant = drupalSettings.gin.toolbar_variant,
        storageDesktop = "Drupal.gin.sidebarExpanded.desktop",
        ...
        (Drupal.ginSidebar = {
          init: function (context) {
            once("ginSidebarInit", "#gin_sidebar", context).forEach(() => {
              localStorage.getItem(storageDesktop) ||
                localStorage.setItem(storageDesktop, "true"),
                window.innerWidth >= 1024 &&
                  ("true" === localStorage.getItem(storageDesktop)
                    ? this.showSidebar()
          ...
          showSidebar: () => {
            const chooseStorage =
                window.innerWidth < 1024
                  ...
                  : storageDesktop,
              hideLabel = Drupal.t("Hide sidebar panel"),
              sidebarTrigger = document.querySelector(".meta-sidebar__trigger");
            (sidebarTrigger.querySelector("span").innerHTML = hideLabel),
              sidebarTrigger.setAttribute("title", hideLabel),
              sidebarTrigger.nextSibling &&
          ...
          collapseSidebar: () => {
            const chooseStorage =
                window.innerWidth < 1024
                  ? "Drupal.gin.sidebarExpanded.mobile"
                  : storageDesktop,
              showLabel = Drupal.t("Show sidebar panel"),
              sidebarTrigger = document.querySelector(".meta-sidebar__trigger");
          ...
          resizeEnd: () => {
            ...
            const setWidth = document.documentElement.style.getPropertyValue(
              ...
              currentWidth = setWidth || resizable.style.width;
            localStorage.setItem("Drupal.gin.sidebarWidth", currentWidth),
          ...
          resizeWidth: (e) => {
            if (isResizing) {
              ...
              sidebarWidth <= 240
                ? (sidebarWidth = 240)
                : sidebarWidth >= 560 && (sidebarWidth = 560),
