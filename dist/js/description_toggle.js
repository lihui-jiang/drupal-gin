  ...
  Drupal.behaviors.formDescriptionToggle = {
    attach: context => {
      context.querySelectorAll(".help-icon__description-toggle").forEach(((elem, index) => {
        ...
        if (container) {
          container.querySelectorAll(".claro-details__description, .fieldset__description, .form-item__description").forEach((description => {
        ...
        elem.addEventListener("click", (event => {
          ...
          const container = event.currentTarget.closest(".help-icon__description-container");
          if (container) {
            container.querySelectorAll(".claro-details__description, .fieldset__description, .form-item__description").forEach(((description, index) => {
              if (index > 1) return;
